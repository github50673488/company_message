...
class Controller_Action
{
	...
	//画面選択
	public function render($viewPath='',$viewName='',$ischeckLogin=true)
	{
		if ($viewPath!='') {$this->viewPath=$viewPath; }
		if ($viewName!='') {$this->viewName=$viewName; }
		if ($ischeckLogin&&!$this->checkLogin()){$this->logoutAction();}
	...
	protected function redirect($controlName,$actionName)
	{
		...
		header('Location: '.$this->getUrl($controlName,$actionName)."&PHPSESSID=".$_REQUEST['PHPSESSID']);
		exit();
	}
	...
	{
		header('Location: '.$url."&PHPSESSID=".$_REQUEST['PHPSESSID']);
		exit();
	}
