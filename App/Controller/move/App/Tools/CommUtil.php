...
class CommUtil {
	...
	public static  function getSendWayStr($sendway)
	{
		$result='';
		switch ($sendway)
		{
	...
	//　引数：メールアドレス文字列
	//	返値：true or false
	public static function agent_check()
	{
		$result='3';
    ...
    public static function check_date($date_str)
	{
        ...
        if(strlen($date_str)!=8){return "日付を確認してください。";}
...
<input type="hidden" name="return"
	...
	public static function Get_log($log, $log_dir="../log/")
	{
		...
		$user = DB_select("s_user", array("user_name"), "WHERE id = ".$_SESSION["login"]["user_id"]." LIMIT 1", false);
		$log_data .= $user["user_name"]."\t";
		$log_data .= Get_codetoname("01", $_SESSION["login"]["office_code"])."\t";
		$log_data .= $log."\r\n";

		Convert_sjis($log_data);
