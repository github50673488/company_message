...
class MessageController extends Controller_Action
{
	...
	function adduserAction() {
		...
		if (count($gp_list)==0){return;}//dbで一グループもない場合
		$user_list=array();
		$gpname_list=array();
		$i=0;
		foreach ($gp_list as $gp)
		{
			...
			if ($gp_list[$i]['is_gp_select'])
			{
				...
				if (count($userOfGroup)>0)
				{
					...
					{
						$userOfGroup[$j]['is_user_select']=$this->findUserSelect($user["user_id"],$_POST["send_userids"]);
		...
		$this->formBean["to_add_user"]=$this->getUrl("Message","adduser");
	...
	function sendAction() {
		...
		$this->formBean["address"]=$isFromUedit?$_SESSION["address"]:"熊本市　小山";
		...
		$this->formBean["to_confirm"]=$this->getUrl("Message","confirm");
		$this->formBean["to_myself"]=$this->getUrl("Message","send");
		$this->formBean["to_add_user"]= $this->getUrl("Message","adduser");
	...
	function confirmAction() {
		if (isset($_POST["testsend"]))
		{
			...
			if (strlen(trim($testSendErr))>0)
			{
				...
				$this->forward("Message","confirmerr");
			}
			else
			{
				...
				CommUtil::getGpsStr($_POST["longitude"],$_POST["latitude"],$_POST["radius"]),
	...
	private function getGpsErr($longitude,$latitude,$radius,$sdate,$edate,$stime,$etime)//dateformat:20090401,timeformat:0940
	{
		...
		if ($longitude<23.420922222222||$longitude>46.085675833333){return "日本国内の緯度を入力して下さい";}
		if ($latitude<122.24856277778||$latitude>149.12034138889){return "日本国内の経度を入力して下さい";}
...
HTMLFILE;
		$filename=info_path.$info_name;//ＨＴＭＬファイル作成		
	...
	private function testSend($user_id,$email,$title,$free_word,$info_name,$isGpsSend,$area,$sdate,$stime,$edate,$etime)
	{
		...
		$this->redirect("Message","finish");//テスト完了
	}
	...
	private function iconcier_send($reserve,$iconcier_users,$sendmode)
	{
		...
		if ($isGps)
		{
			$area=CommUtil::getGpsStr($_POST["longitude"],$_POST["latitude"],$_POST["radius"]);
