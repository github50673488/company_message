...
class ScheduleModel {
   ...
   function get_file_title($fileName)
	...
	function get_ipass($id) {
		$mysqli = DBConn::getConn ();
		$sql = "select i_password from schedule where id = ".$id." AND kind =1 ";
		$ipass="";
		if ($result = $mysqli->query($sql)) {
			$row = $result->fetch_array();
			$ipass=$row[0];
	...
	function select($where,$page) {
		...
		a.title,a.place,a.detail,a.parentid, create_user,a.update_user,a.filename,a.create_date as idx1
		 from schedule a
		 ".
	...
	/*function i_register($registerArray, $s_id) {
		$mysqli = DBConn::getConn ();
		$sql = "insert into schedule (s_id,create_date,kind,title,update_user,create_user)
		values(?, now(), ?,?,?,?) ;";
		$stmt = $mysqli->prepare ($sql);
		$stmt->bind_param ( "sisss", $s_id, $registerArray ['kind'], $registerArray ['title'],
		$registerArray ['update_user'],$registerArray ['create_user'] );
		$stmt->execute ();
		$insert_id=$mysqli->insert_id;
		$mysqli->close ();
		return $insert_id;
	...
	function del($id) {
		...
		if ($stmt->execute ())
		{
			$filePath=schedule_path.$filename;
	...
	/*function update_i_schedule_title($updateArray) {
		$mysqli = DBConn::getConn ();
		$sql = " update schedule set title=?,update_user=? where id=? ;";
		$stmt = $mysqli->prepare ( $sql );
		$stmt->bind_param ("ssi", $updateArray ['title'],
		$updateArray ['update_user'], $updateArray ['id']);
		$stmt->execute ();
		$mysqli->close ();
		}*/
	...
	function send_iSuhedule($iSuhedule, $suhedule_id, $id, $up_type){
		...
		$data_file = schedule_path.$filename;  // ファイル名
